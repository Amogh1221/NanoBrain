...
In a causal (or auto-regressive) language model, the network parameterizes the conditional probability distribution $P(x_t \mid x_{<t})$ over a finite vocabulary set $V$:
...
<p align="center">
  <img src="docs/images/rnn_vs_transformer.svg" alt="Sequential RNN Bottleneck vs Parallel Transformer Self-Attention" width="680">
  <br>
...
<p align="center">
  <img src="docs/images/gpt_architecture.svg" alt="Full GPT Decoder Architecture Vector SVG" width="400">
  <br>
...
<p align="center">
  <img src="docs/images/positional_encoding.svg" alt="Positional Encoding Matrix Vector SVG" width="650">
  <br>
...
<p align="center">
  <img src="docs/images/multi_head_attention.svg" alt="Multi-Head Attention Vector SVG" width="650">
  <br>
...
<p align="center">
  <img src="docs/images/activation_functions.svg" alt="Activation Functions Comparison Vector Plot" width="600">
  <br>
...
<p align="center">
  <img src="docs/images/flash_attention_tiling.svg" alt="GPU SRAM vs HBM Memory Hierarchy & FlashAttention Tiling Vector SVG" width="650">
  <br>
...
In NanoBrain ([`model.py`](model.py)), FlashAttention is invoked natively via PyTorch 2.0's:
...
<p align="center">
  <img src="docs/images/learning_rate_schedule.svg" alt="Cosine Annealing Learning Rate Schedule Vector Plot" width="600">
  <br>
...
In NanoBrain ([`model.py`](model.py)), weight decay ($\lambda = 0.1$) is applied exclusively to 2D linear weight matrices, while 1D LayerNorm weights and bias vectors are explicitly excluded.
...
Training stochasticity causes model parameters $\theta_t$ to oscillate around local minima. NanoBrain includes an **Exponential Moving Average (EMA)** module ([`model.py`](model.py)) that maintains a smoothed shadow copy of parameters $\theta_{EMA}$:
...
<p align="center">
  <img src="docs/images/decoding_sampling.svg" alt="Temperature Scaling & Truncation Sampling Vector SVG" width="680">
  <br>
...
<p align="center">
  <img src="docs/images/gqa_mha_mqa.svg" alt="Attention Head Architecture Comparison Vector SVG" width="650">
  <br>
...
### 6.1 Hyperparameter Configuration ([`config.py`](config.py) / [`config.json`](config.json))

The `GPTConfig` dataclass in [`config.py`](config.py) encapsulates architectural, training, dataset, and system settings.
...
### 6.2 Data Sourcing and Pre-Processing ([`build_dataset.py`](build_dataset.py))

[`build_dataset.py`](build_dataset.py) constructs a high-quality pre-training text corpus by streaming and mixing 5 datasets:
...
### 6.3 Binary Packing and Zero-Copy Data Loading ([`tokenize_dataset.py`](tokenize_dataset.py) and [`dataset.py`](dataset.py))

#### 1. Binary Pre-Tokenization ([`tokenize_dataset.py`](tokenize_dataset.py))
...
#### 2. Zero-Copy Memory Mapping ([`dataset.py`](dataset.py))
[`dataset.py`](dataset.py) defines `BinDataset`, leveraging `np.memmap` to interface with binary dataset files:
...
### 6.4 PyTorch Model Implementation ([`model.py`](model.py))

[`model.py`](model.py) implements the Transformer architecture components:
...
### 6.5 Unified Training Engine ([`trainer.py`](trainer.py) and [`train.py`](train.py))

[`trainer.py`](trainer.py) manages the training execution loop:
...
### 6.6 Text Generation Script ([`generate.py`](generate.py))

[`generate.py`](generate.py) runs model inference:
